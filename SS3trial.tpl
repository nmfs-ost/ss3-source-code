      ...
      for (i=1;i<=Svy_N_rd;i++)  // loop all, including those out of yr range
      {
        ...
        if(y>=styr && y<=endyr)
        {
          ...
          {  //  start have_data index and timing processing
            ...
            data_timing=get_data_timing(N_subseas, Svy_data(i)(1,3), seasdur, subseasdur_delta, azero_seas);
            cout<<" data_timing from routine "<<data_timing<<" y "<<y<<endl;
...
//  SS_Label_Info_10.3  #start random number generator with seed based on time
  ...
  double get_data_timing(int N_subseas, dvector Y_Mo_F, dvector seasdur, dvector subseasdur_delta, dvector azero_seas)
  {
    int t,s,subseas,yr,month,fleet;
    double temp, temp1, data_timing;
    yr=Y_Mo_F(1);
    month=Y_Mo_F(2);
    fleet=Y_Mo_F(3);
              cout<<" y-mo-fleet  "<<yr<<" "<<month<<" "<<fleet<<endl;
              temp1=(abs(month)-1.0)/12.;  //  month as fraction of year
              s=1;  // earlist possible seas;
              subseas=1;  //  earliest possible subseas in seas
              temp=subseasdur_delta(s);  //  starting value
              while(temp<=temp1)
