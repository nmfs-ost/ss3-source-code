          ...
          case 1:  // breakpoints
          {
            ...
            for (s=1;s<=nseas;s++)
            {
              ...
              for (loop=1;loop<=N_natMparms+1;loop++)
              {
                ...
                {temp=(natM_B-natM_A)/(natM_amax-natM_amin);}  //  calc the slope
                else
                {temp=0.0;}
                while(t_age<natM_amax && a<=nages)
                {
                  natM_M1(s,gpi,a)=natM_A+(t_age-natM_amin)*temp;
                  t_age+=1.0; a++;
                }
              }
              if(k==1) natM_M1(s,gpi,0)=natM_M1(s,gpi,1);
          ...
          case(4):
          {
            for (s=1;s<=nseas;s++)
            {
              if(s>=Bseas(g))
              {
                ...
                for (a=k;a<=nages-1;a++)
                {
                  natM_M1(s,gpi,a) = Age_NatMort(gp,a)+t_age*(Age_NatMort(gp,a+1)-Age_NatMort(gp,a));
                } // end age
              }
              else
              {
...
//  natM_M1(s,gpi,a) = Maunder_Mjuv*pow(Ave_Size(t,ALK_idx,g,a)/Maunder_Lmat,Maunder_lambda) +
//                  (Maunder_Mmat-Maunder_Mjuv*pow(Ave_Size(t,ALK_idx,g,a)/Maunder_Lmat,Maunder_lambda))*XXmaturity_Fem(a)XX;
        	    		natM_M1(s,gpi) = Maunder_Mjuv*pow((Ave_Size(t,mid_subseas,g)/Maunder_Lmat),Maunder_lambda);
        	    		natM_M1(s,gpi) += elem_prod((Maunder_Mmat-natM_M1(s,gpi)),XX_mature);
        	    	}
        	    	if(do_once==1)
        	    		{
                    ...
                    echoinput<<"avesize/Lmat "<<Ave_Size(t,mid_subseas,g)/Maunder_Lmat<<endl;
        	      		...
        	      		echoinput<<" natM_combined: "<<natM_M1(s,gpi)<<endl;
        	    		}
        	    }
          	break;
          }
  ...
  //  SS_Label_Info_17.2  #calc an ave_age for the first gp as a scaling factor in logL for initial recruitment (R1) deviation
        ...
        if(do_once==1)
        {
         for(s=1;s<=nseas;s++) echoinput<<"Natmort seas:"<<s<<" sex:"<<gg<<" Gpat:"<<GPat<<" sex*Gpat:"<<gp<<" settlement:"<<settle<<" gpi:"<<gpi<<endl<<" M: "<<natM_M1(s,gpi)<<endl;
        }
 #endif
  ...
  if(sx(g)==1 && use_morph(g)>0)
  {
    ...
    switch(Maturity_Option)
    {
      case 1:  //  Maturity_Option=1  length logistic
     {
      ...
      if(t>=styr && WTage_rd==0) Wt_Age_t(t,-2,g)= fec(g);   //  save sel_num and save fecundity for output
      if(y==endyr && WTage_rd==0) Wt_Age_t(t+nseas,-2,g)=fec(g);
 #ifdef DO_ONCE
     if(do_once==1){
  ...
  /*  SS_Label_Function_21 #get_saveGparm */
    ...
    for (GPat=1;GPat<=N_GP;GPat++)
    {
      ...
      for (settle=1;settle<=N_settle_timings;settle++)
      {
        ...
        save_G_parm(save_gparm,15)=value(natM_M1(1,GP3(g),0));
        save_G_parm(save_gparm,16)=value(natM_M1(1,GP3(g),nages));
        if(gg==1)
        {
