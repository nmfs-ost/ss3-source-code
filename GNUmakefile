...
ifdef USE_DOCKER
  ifeq ($(OS),Windows_NT)
    docker run --rm --volume $(CURDIR):C:\\workdir\\ss3 --workdir C:\\workdir\\ss3 johnoel/admb-13.2:windows10 ss3.tpl
  else
	docker run --rm --volume $(CURDIR):/workdir/ss3:rw --workdir /workdir/ss3 johnoel/admb-13.2:linux ss3.tpl
  endif
else
...
ifdef USE_DOCKER
  ifeq ($(OS),Windows_NT)
	docker run --rm --volume $(CURDIR):C:\\workdir\\ss3_opt --workdir C:\\workdir\\ss3_opt johnoel/admb-13.2:windows10 ss3_opt.tpl
  else
	docker run --rm --volume $(CURDIR):/workdir/ss3_opt:rw --workdir /workdir/ss3_opt johnoel/admb-13.2:linux ss3_opt.tpl
  endif
else
