...
ss: ss.tpl
ifdef USE_DOCKER
	docker run --rm --volume $(CURDIR):/ss:rw --workdir /ss johnoel/admb:linux ss.tpl
else
...
ss_opt: ss_opt.tpl
ifdef USE_DOCKER
	docker run --rm --volume $(CURDIR):/ss_opt:rw --workdir /ss_opt johnoel/admb:linux ss_opt.tpl
else
