...
ss: ss.tpl
ifdef USE_DOCKER
	docker run --rm --volume $(CURDIR):/workdir/ss:rw --workdir /workdir/ss --entrypoint ls johnoel/admb:linux -al /tmp/ss
	docker run --rm --volume $(CURDIR):/workdirtmp/ss:rw --workdir /workdir/ss johnoel/admb:linux ss.tpl
else
