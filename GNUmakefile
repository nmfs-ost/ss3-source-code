...
ifdef USE_DOCKER
  ifeq ($(OS),Windows_NT)
	docker run --rm --volume $(CURDIR):/workdir/ss --workdir /workdir/ss johnoel/admb:linux ss.tpl
  else
...
ifdef USE_DOCKER
  ifeq ($(OS),Windows_NT)
	docker run --rm --volume $(CURDIR):/workdir/ss_opt --workdir /workdir/ss_opt johnoel/admb:linux ss_opt.tpl
  else
