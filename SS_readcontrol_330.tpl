  ...
  if(N_Block_Designs>0)
  {
    ...
    for (j=1;j<=N_Block_Designs;j++)
    {
      ...
      for (k=1;k<=Nblk2(j)/2;k++)
      {
        ...
        if(Block_Design(j,a+1)>retro_yr+1) {N_warn++; warning<<"Block:"<<j<<" "<<k<<" ends after retroyr+1; resetting to: "<<retro_yr+1<<endl; Block_Design(j,a+1)=retro_yr+1;}
...
//  8=dev vector used; 9=dev link type; 10=dev min year; 11=dev maxyear; 12=dev phase; 13=all parm index of baseparm
       ...
       if(autogen_timevary(2)==0)
        {echoinput<<" timevary parms will be autogenerated "<<endl;}
        else
        {echoinput<<" timevary parms will be read, not autogenerated "<<endl;}
       timevary_setup(2)=j; //  index of base parm within that type of parameter
