                ...
                switch(Svy_units(f))
                {
                  case 1:  //  biomass
                  {
                    if(WTage_rd==1)  //  using empirical wt-at-age;  note that this cannot use GP specific bodyweights
                    {
                      ...
                      {
                       for (a=0;a<=nages;a++) vbio+=Wt_Age_all(y,f,1,a)*agetemp(a);
                       if(gender==2)
                       {
                        for (a=0;a<=nages;a++) vbio+=Wt_Age_all(y,f,2,a)*agetemp(a+nages+1);
                       }
                      }
                      else
                      {
                       for (a=0;a<=nages;a++) vbio+=Wt_Age_all(y,f,1,a)*exp_truea_ret(a);
                       if(gender==2)
                       {
                        for (a=0;a<=nages;a++) vbio+=Wt_Age_all(y,f,2,a)*exp_truea_ret(a+nages+1);
                       }
                      }
                    }
                    else
