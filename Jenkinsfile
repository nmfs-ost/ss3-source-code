pipeline {
    ...
    stages {
        ...
        stage('Build SS Executable') {
            steps {
                sh label: 'view folder structure', script: 'ls /usr/local/admb-12.1 -R'
                ...
                sh label: 'Run build', script: 'cd vlab/stock-synthesis && /bin/bash ./Make_SS_330_new.sh -b SS330 -a /usr/local/admb-12.1/admb'
