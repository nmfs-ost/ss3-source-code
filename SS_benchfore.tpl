...
//  following uses the values of sel_l, sel_a, etc. stored in yr=styr-3
      ...
      for(g=1;g<=gmorph;g++) {
        ...
        {
          tempvec_a.initialize();
          for(f=1;f<=Nfleet;f++) {tempvec_a+=Bmark_RelF_Use(s,f)*sel_dead_num(s,f,g);}
          temp=max(tempvec_a);
...
//  else using the bioecon options that depend on MSY_units
        ...
        if(j<=9)
          {F2(1)=(1.-Closer)*temp+Closer*(bestF1/bestF2);}        // averages with best value to keep from changing too fast
        else
          {F2(1)=temp;}
        }   // end search loop
                    ...
                    if(Fcast_InputCatch(t,f,2)<=3)  //  input is catch
                      {
                        ...
                        {
                          Tune_F_loops=8;
                          if(Fcast_RelF_Use(s,f)>0.0) Do_F_tune(t,f)=1;
                        }
                      }
                    else
                      ...
                      if(use_morph(g)>0)
                      {
                        if(catchunits(f)==1)  //  catch in weight
                        {
                          if(Fcast_InputCatch(t,f,2)==2)
                            {temp+=Nmid(g)*sel_dead_bio(s,f,g);}      // dead catch bio
                          else if(Fcast_InputCatch(t,f,2)==3)
                            {temp+=Nmid(g)*sel_ret_bio(s,f,g);}      // retained catch bio
                        }
                        else   //  catch in numbers
                        {
                          if(Fcast_InputCatch(t,f,2)==2)
                          {temp+=Nmid(g)*sel_dead_num(s,f,g);}      // deadfish catch numbers
                          else if(Fcast_InputCatch(t,f,2)==3)
                          {temp+=Nmid(g)*sel_ret_num(s,f,g);}      // retained catch numbers
                        }
                      }  //close gmorph loop
                  ...
                  if (Do_F_tune(t,f)==1)  // have an input catch or in ABC_loop 3, so get expected catch from F and Z
                  {
                    ...
                    if(ABC_Loop==2)  //  tune to input catch in ABCloop 2;  Do_F_tune(t,f) is only turned on if there is input catch
                    {
                      ...
                      if(use_morph(g)>0)
                      {
                        ...
                        {
                          if(Fcast_InputCatch(t,f,2)==2)
                            {C_temp(f)+=elem_prod(natage(t,p,g),sel_dead_bio(s,f,g))*Zrate2(p,g);}      // dead catch bio
                          else if(Fcast_InputCatch(t,f,2)==3)
                            {C_temp(f)+=elem_prod(natage(t,p,g),sel_ret_bio(s,f,g))*Zrate2(p,g);}      // retained catch bio
                        }
                        else   //  catch in numbers
                        {
                          if(Fcast_InputCatch(t,f,2)==2)
                          {C_temp(f)+=elem_prod(natage(t,p,g),sel_dead_num(s,f,g))*Zrate2(p,g);}      // deadfish catch numbers
                          else if(Fcast_InputCatch(t,f,2)==3)
                          {C_temp(f)+=elem_prod(natage(t,p,g),sel_ret_num(s,f,g))*Zrate2(p,g);}      // retained catch numbers
                        }
                      }  //close gmorph loop
...
//            report5<<Tune_F<<" tune_area"<<Fcast_Catch_Calc_Annual<<endl;
          ...
          if(Fcast_Catch_Allocation_Groups>0  && y>=Fcast_Cap_FirstYear)  // adjust to get a specific fleet allocation
          {
            ...
            {
              f=fish_fleet_area(0,ff);
              if (Allocation_Fleet_Assignments(f)==g) {Fcast_Catch_Allocation_Group(g)+=Fcast_Catch_Calc_Annual(f);}
            }
