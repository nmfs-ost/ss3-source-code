...
FUNCTION void write_nudata()
{
  ...
  for (Nudat = 1; Nudat <= N_nudata; Nudat++) {
    ...
    if (Nudat == 1) // report back the input data
    {
      ...
      report1 << "#" << endl
              << "# Sizefreq data. Defined by method because a fleet can use multiple methods" << endl;
...
FUNCTION void write_nucontrol()
{
  ...
  NuStart << depletion_basis_rd << " # Depletion basis:  denom is: 0=skip; 1=X*SSBvirgin; 2=X*SSBmsy; 3=X*SSB_styr; 4=X*SSB_endyr; 5=X*dyn_Bzero;  values>=11 invoke N multiyr with 10s & 100s digit; append .1 to invoke log(ratio); e.g. 122.1 produces log(12 year trailing average of B/Bmsy)" << endl;
  ...
  anystring = "";
  if (Fcast_yr_rd(1) != -12345) { //  write in old format
  ...
  { //  new list based format for Fcast years
    NuFore << anystring << endl
           << anystring << "-12345  # code to invoke new format for expanded fcast year controls" << endl
    ...
    NuFore << anystring << "-9999 0 0 0" << endl
           << "#" << endl;
  }
  ...
  NuFore << "#" << endl
         << Fcast_Loop_Control(1) << " #_N forecast loops (1=OFL only; 2=ABC; 3=get F from forecast ABC catch with allocations applied)" << endl;
  ...
  NuFore << Fcast_Loop_Control(5) << " # HCR_anchor: 0 or 1 uses unfished benchmark SSB (old hardwired approach), 2 = virgin SSB" << endl
         << "#" << endl;
  ...
  report4 << SR_update_SSBpR0_rd << "  #  SR_update_SSBpR0" << endl
          ...
          << "#  3 - option:  do not update SSBpR0 (do keep start year SSBpR0), even if R0 or h is set to have time-varying property" << endl
          << "#" << endl;
  ...
  if (F_Method == 1) {
    ...
    report4 << "# (A) fleet;" << endl
            << "# (B) F_starting_value (ignored if start_PH=1 or reading from ss3.par);" << endl
            << "# (C) start_PH for fleet's Fparms (99 to stay in hybrid, <0 to stay at starting value)" << endl
